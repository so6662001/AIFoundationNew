...
评审用原型，无需构建，浏览器直接打开 **`overview.html`**（总览：所有页面实时缩略 + 说明 + 深链）或 `index.html`。所有交互为前端模拟（无后端），用于确认信息架构、页面布局、节点与面板的字段设计。

| 页面 | 文件 | 演示要点 |
|---|---|---|
| 原型总览 | `overview.html` | 9 个页面实时缩略（iframe 缩放）、角色分组、主路径图、深链入口、与方案文档的对应关系 |
| 运行列表 | `runs.html` | 状态快捷筛选、流程 / 版本 / 触发 / 时间过滤、节点进度条、子运行折叠、一键重试 / 审批 |
| 流程度量 | `metrics.html` | 运行数与成功率趋势、节点耗时 P50/P95、分支命中分布、审批时长分布、失败原因 Top、失败率热力图 |
| 节点注册中心 | `registry.html` | 节点类型列表（版本 / 队列 / 语言 / Worker / 执行统计）、Schema 详情、Worker 心跳、被引用流程、"如何新增节点"Java 示例 |
